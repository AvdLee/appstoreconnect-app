...
final class AddTestFlightUserViewController: NSViewController {
    ...
    @IBAction func addButtonTapped(_ sender: NSButton) {
        APIProvider.shared.request(APIEndpoint.create(betaTesterWithEmail: emailTextField.stringValue, firstName: firstNameTextField.stringValue, lastName: lastNameTextField.stringValue, betaGroupIds: [])) { (result) in
            switch result {
